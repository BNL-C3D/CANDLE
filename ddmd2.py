...
class DDMD(object):
    ...
    # define pilot size, keep track of core usage
    cores_pilot = 128
    cores_used  =   0
    ...
    def __init__(self):
        ...
        # bookkeeping
        self._aggregated     =  0
        self._aggregated_max = 16  # aggregation threshold
    ...
    def _submit_task(self, ttype, n=1):
        ...
        NOTE: all tasks are uniform for now: they use a single core and sleep
              for a random number (0..10) of seconds.
        '''
        ...
        with self._lock:
            ...
            for _ in range(n):
                tds.append(rp.TaskDescription({
                         ...
                         'arguments'    : [str(random.randint(1,5))]}))
    ...
    def _state_cb(self, task, state):
        ...
        if state in [rp.TMGR_SCHEDULING] + rp.FINAL:
            self._rep.info(' <<%s -> %s' % (task.uid, task.state))
        ...
        with self._lock:
            ...
            if state == rp.FAILED:
                print('ERROR: task %s failed: %s' % (task.uid, task.stderr))
